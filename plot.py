...
def plot_heatmap(df):
    ...
    for row in range(n_rows):
        for column in range(n_columns):
            ...
            df_setting = df_setting.loc[df_setting.groupby([axes[0], axes[1]])["epsilon"].idxmax()]
            with pd.option_context('display.max_rows', None, 'display.max_columns', None):
                print(df_setting)
            df_setting = pd.pivot_table(df_setting, values=values[0], index=[axes[0]], columns=[axes[1]], aggfunc=np.max)
            sns.heatmap(df_setting, annot=True, fmt=".2%", vmax=value_range[1], vmin=value_range[0])
            # subplot titles
